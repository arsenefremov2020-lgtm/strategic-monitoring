...
with st.expander("🔧 Панель адміністратора — деталізація всіх розрахунків", expanded=False):
    ...
    # ── 1. Джерела даних ──
    st.markdown("### 1. Джерела даних")

    a1, a2 = st.columns(2)
    with a1:
        st.markdown('<div class="section-card">', unsafe_allow_html=True)
        st.markdown('<div class="section-title">📁 Excel (стратегічна матриця)</div>', unsafe_allow_html=True)
        total_rows = len(strat_df)
        meas_total = len(strat_df[strat_df["object_type"] == "measure"])
        goals_total = len(strat_df[strat_df["object_type"] == "goal"])
        tasks_total = len(strat_df[strat_df["object_type"] == "task"])
        active_in_year = len(evaluation_df)

        st.markdown(f"""
        <div class="methodology-box">
        <strong>Файл:</strong> {FILE_PATH}<br>
        <strong>Аркуш:</strong> {SHEET_NAME}<br>
        <strong>Зчитано рядків (з 8-го):</strong> {total_rows}<br>
        <strong>Цілей:</strong> {goals_total} &nbsp;|&nbsp; <strong>Завдань:</strong> {tasks_total} &nbsp;|&nbsp; <strong>Заходів:</strong> {meas_total}<br>
        <strong>Активних заходів у {selected_year} р.:</strong> {active_in_year}<br>
        <strong>Колонки:</strong> type_marker(B), code(C), name(D), indicator(F), unit(G),
        base_2021(H), fact_2024(I), fact_2025(J), target_2026–2028(K–M), department(R)
        </div>
        """, unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with a2:
        st.markdown('<div class="section-card">', unsafe_allow_html=True)
        st.markdown('<div class="section-title">🗄 Supabase (моніторингові подання)</div>', unsafe_allow_html=True)
        ...
        <strong>Записів у таблиці (всього):</strong> {total_supabase}<br>
        <strong>Фільтр:</strong> рік = {selected_year}, квартал = {selected_quarter}, approval_status = «Погоджено»<br>
        <strong>Після фільтрації:</strong> {total_filtered} записів<br>
        <strong>Унікальних кодів заходів із даними:</strong> {unique_codes_mon}<br>
        <strong>Заходів без погоджених даних:</strong> {active_in_year - approved_measures}
        </div>
        """, unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    if not evaluation_df.empty and not task_scores.empty and not goal_scores.empty:
        ...
        w_tasks    = round(float(task_scores["task_score"].mean()), 2)
        w_goals    = round(float(goal_scores["goal_score"].mean()), 2)
        ...
        agg_rows.append({
            ...
            "Формула": "Факт/План×100 або статус"
        })
        agg_rows.append({
            "Рівень": "Завдання",
            "Кількість": len(task_scores),
            "Середня оцінка, %": w_tasks,
            "Вага": "30%",
            "Внесок в інтегральну": f"{round(w_tasks * 0.30, 2)}%",
            "Формула": "MEAN(оцінки заходів завдання)"
        })
        agg_rows.append({
            "Рівень": "Стратегічна ціль",
            "Кількість": len(goal_scores),
            "Середня оцінка, %": w_goals,
            "Вага": "50%",
            "Внесок в інтегральну": f"{round(w_goals * 0.50, 2)}%",
            "Формула": "MEAN(оцінки завдань цілі)"
        })
        agg_rows.append({
            ...
            "Формула": f"{w_measures}×0.20 + {w_tasks}×0.30 + {w_goals}×0.50 = {integral_score}%"
        })
