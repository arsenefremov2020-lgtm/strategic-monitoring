...
# ============================================================
# FLOATING CHAT WITH BIRD — components.html (true floating)
# ============================================================

import streamlit.components.v1 as _components

# Завантажуємо повідомлення для всіх трьох каналів одразу
...
def _msgs_to_js(msgs):
    """Convert list of message dicts to JS array literal."""
    import json
    safe = []
    for m in msgs:
        safe.append({
...
# Підрахунок непрочитаних (нових за останню годину)
from datetime import timezone, timedelta
_now = datetime.now(timezone.utc)
_unread = sum(
    1 for ch_msgs in _all_msgs.values()
    for m in ch_msgs
    if m.get("sender", "") != "Керівник ССП"
    and (_now - datetime.fromisoformat(m["created_at"].replace("Z", "+00:00"))).seconds < 3600
) if any(_all_msgs.values()) else 0
...
#fab:hover {{ transform: scale(1.09); box-shadow: 0 12px 32px rgba(37,99,235,.55); }}
...
#fab-badge {{
  ...
  border: 2px solid #fff;
  display: {("flex" if _unread > 0 else "none")};
}}
...
.ctab.active {{ color: #1e40af; border-bottom-color: #3b82f6; background: #fff; }}
...
.empty-msg {{ color: #94a3b8; font-size: 13px; text-align: center; margin: auto; padding: 20px; }}
...
#chat-sending {{ color: #94a3b8; font-size: 11px; padding: 4px 12px; display: none; flex-shrink: 0; }}
</style>
</head>
<body>

<!-- FAB -->
<div id="fab" onclick="toggleChat()" title="Відкрити чат">
  <div id="fab-badge">{_unread if _unread > 0 else ""}</div>
...
<div id="chat">
  ...
  <div id="chat-tabs">
    <div class="ctab active" onclick="switchTab('coordinator')" id="tab-coordinator">📋 Координатор</div>
    <div class="ctab" onclick="switchTab('responsible')" id="tab-responsible">👤 ССП</div>
    <div class="ctab" onclick="switchTab('group')" id="tab-group">👥 Груп</div>
  </div>

  <div id="chat-messages"></div>
  <div id="chat-sending">Надсилання...</div>
...
// Pre-loaded messages (from Python/Supabase at page render)
const PRELOADED = {{
...
let currentChannel = "coordinator";
let polling = null;
...
function toggleChat() {{
  ...
  if (win.classList.contains("open")) {{
    ...
    renderMessages(PRELOADED[currentChannel]);
    scrollBottom();
    startPolling();
...
function renderMessages(msgs) {{
  ...
  if (!msgs || msgs.length === 0) {{
    box.innerHTML = '<div class="empty-msg">Повідомлень поки немає.<br>Напишіть першим 👇</div>';
    return;
  }}
  box.innerHTML = msgs.map(m => {{
    const isMe = m.sender === MY_SENDER;
    const col  = AVATAR_COLORS[m.sender]  || AVATAR_COLORS["default"];
    const lett = AVATAR_LETTERS[m.sender] || AVATAR_LETTERS["default"];
    return `
      <div class="msg-wrap ${{isMe ? 'me' : 'other'}}">
        <div class="msg-row">
          <div class="avatar" style="background:${{col}}">${{escHtml(lett)}}</div>
          <div class="bubble">${{escHtml(m.text)}}</div>
        </div>
        <div class="msg-meta">${{escHtml(m.sender)}} · ${{escHtml(m.ts)}}</div>
      </div>`;
  }}).join("");
}}
...
// ── Fetch latest from Supabase REST ──
async function fetchMessages() {{
  try {{
    const url = SUPABASE_URL + "/rest/v1/chat_messages?channel=eq." +
...
async function sendMessage() {{
  ...
  inp.value = "";
  inp.disabled = true;
  document.getElementById("chat-sending").style.display = "block";
...
// ── Auto-polling every 8s while open ──
function startPolling() {{
  stopPolling();
  polling = setInterval(fetchMessages, 8000);
}}
...
_components.html(_chat_html, height=0, scrolling=False)
