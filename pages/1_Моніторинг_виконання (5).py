...
# Блокуємо повторне подання: department = selected_ssp_index, рік, квартал, код
...
for _, row in filtered_measures.iterrows():
    ...
    # Визначаємо роль цього ССП у заході
    ...
    # Заступник Міністра за головним виконавцем заходу
    deputy = get_deputy_minister_for_ssp(extract_ssp_index(row.get("resp_main", "")))

    # Чи вже подано заявку за цим заходом у поточному кварталі
    ...
    table_rows.append({
        ...
        "Відповідальні самостійні\nструктурні підрозділи\nГоловний виконавець": raw_value(row.get("resp_main", "")),
        ...
        "Заступник\nМіністра": deputy,
        "Роль ССП\nу заході": role_label,
        "Статус у\nсистемі": "✅ Подано" if is_already_submitted else "",
...
# Якщо є вже подані заходи — виводимо нотатку
if already_submitted_codes and not table_df.empty:
    already_in_table = [c for c in already_submitted_codes if c in table_df["Код"].values]
    if already_in_table:
        st.markdown(
            f"""
            <div class="note-box" style="background:#fef9c3;border:1px solid #fde047;color:#713f12;">
                ...
                у {quarter_label} вже подано (позначено ✅ Подано у колонці «Статус у системі»). 
...
if table_df.empty:
    ...
    edited_df = st.data_editor(
        ...
        column_config={
            ...
            "Відповідальні самостійні\nструктурні підрозділи\nГоловний виконавець": st.column_config.TextColumn(
                "Відповідальні самостійні\nструктурні підрозділи\nГоловний виконавець",
                disabled=True,
                width=220
            ),
            ...
            "Заступник\nМіністра": st.column_config.TextColumn(
                "Заступник\nМіністра",
                disabled=True,
                width=220
            ),
            "Роль ССП\nу заході": st.column_config.TextColumn(
                "Роль ССП\nу заході",
                help="Роль обраного ССП у цьому заході",
                disabled=True,
                width=170
            ),
            "Статус у\nсистемі": st.column_config.TextColumn(
                "Статус у\nсистемі",
                help="✅ Подано — відомості за цей квартал вже надіслано",
                disabled=True,
                width=130
            ),
...
def validate_submission():
    ...
    for _, row in selected_rows.iterrows():
        ...
        # Блокуємо повторне подання за той самий захід і квартал
