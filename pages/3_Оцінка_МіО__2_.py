...
def _rv_fmt_num(val):
    """Форматує бал 0–1 в український стиль: 1 → «1», 0.75 → «0,75»."""
...
def _rv_year_cell(val):
    ...
    if isinstance(val, (int, float)):
        ...
        return (
            '<td class="m-ratio">'
            f'<div class="rbar {cls}"><div class="rfill" style="width:{width:.0f}%;"></div>'
            f'<span class="rlabel">{_rv_fmt_num(val)}</span></div></td>'
        )
    if val == "в/а":
        return '<td class="m-ratio"><span class="tchip obsolete" title="Втратило актуальність">в/а</span></td>'
...
def _build_rv_table_html(df):
    ...
    <table class="mio-table">
      <thead>
        <tr class="grp">
          <th class="m-anchor sticky-h" rowspan="2">Захід</th>
          <th colspan="3" class="grp-q">Бал виконання (наростаючим)</th>
          <th rowspan="2" class="grp-year">Бал за РІК</th>
...
def _render_rv_meas_year(strat_df, monitoring_df, year):
    ...
    n_total = len(df_full)
    # Середній бал за РІК (лише числові значення)
    num_year = [v for v in df_full["Бал · РІК"] if isinstance(v, (int, float))]
    avg_year = round(sum(num_year) / len(num_year), 2) if num_year else None
    ...
    kpis = "".join([
        ...
        kpi_card(
            "Середній бал за РІК",
            _rv_fmt_num(avg_year) if avg_year is not None else "—",
            "gray"
        ),
    ])
    st.markdown(
    ...
    st.markdown(f"""
    <div class="mio-legend">
        <span class="mio-chip done"><span class="dot"></span>Виконано (бал &ge; 1)</span>
        <span class="mio-chip partial"><span class="dot"></span>Частково виконано (0,75 &lt; бал &lt; 1)</span>
        <span class="mio-chip notdone"><span class="dot"></span>Не виконано (бал &le; 0,75)</span>
    ...
    for col in ["Бал · I кв", "Бал · I пів", "Бал · 9 міс", "Бал · РІК"]:
        csv[col] = csv[col].map(lambda v: _rv_fmt_num(v) if isinstance(v, (int, float)) else v)
    st.download_button(
        f"⬇️ Завантажити таблицю за {year} рік (CSV)",
        data=csv.to_csv(index=False).encode("utf-8-sig"),
...
def render_mode_rv_meas(strat_df, monitoring_df, years):
    ...
    st.markdown(
        "<div style='font-size:13px;color:#475569;margin-bottom:8px;line-height:1.5;'>"
        "Бали виконання переводяться зі станів режиму <b>«М_заходи»</b> за довідником моделі: "
        "<b>Виконано → 1</b>, <b>Частково виконано → 0,75</b>, <b>Не виконано → 0</b>, "
        "<b>Не настав час → «х»</b>, <b>Втратило актуальність → «в/а»</b>. "
        "Бал за <b>РІК</b> дорівнює співвідношенню Факту і Плану ÷ 100, а "
        "<b>Кінцевий результат</b> класифікує цей бал (≥ 1 — виконано; 0,75–1 — частково; інакше — не виконано)."
        "</div>",
        unsafe_allow_html=True
    )
    if len(years) == 1:
