...
def load_strat_matrix():
    ...
    result = pd.DataFrame({
        ...
        "budget_kpkvk": safe_col(24),         # КПКВК ДБ
        "budget_2026": safe_col(25),           # ДБ 2026, затверджено, млрд грн
        "budget_2027": safe_col(26),           # ДБ 2027, прогноз, млрд грн
        "budget_2028": safe_col(27),           # ДБ 2028, прогноз, млрд грн
        "other_source": safe_col(28),          # Інше джерело (МТД, МБРР, партнери...)
        "other_2026": safe_col(29),            # Інше джерело 2026, план
        "other_2027": safe_col(30),            # Інше джерело 2027, прогноз
        "other_2028": safe_col(31),            # Інше джерело 2028, прогноз
    })
...
def apply_dashboard_filters(active, department_indices, goals, tasks, measures, product_types, deputies, statuses, sources, financing_types=None, kpkvk_codes=None):
    ...
    if sources:
        # source_national може містити кілька документів через кому + номери + (Ін) + лапки
        # Шукаємо кожен обраний документ як підрядок у полі source_national
        def _source_match(val):
            v = str(val)
            return any(doc in v for doc in sources)
        data = data[data["source_national"].apply(_source_match)]
    if financing_types:
...
source_options = sorted([
    ...
    "План пріоритетних дій Уряду на 2026 рік",
    "План України",
...
if not presentation_mode:
    ...
    with fin_col2:
        ...
        if "goal_code" in fin_measures.columns and "budget_2026" in fin_measures.columns:
            goal_budget = (
                fin_measures[fin_measures["budget_2026"].notna()]
                .groupby("goal_code")
                .agg(
