                ...
                try:
                    ...
                    if auto_completed_list:
                        _ins.append(f"⚡ Авто-зараховано системою: {len(auto_completed_list)} захід(ів) "
                                    f"(значення ≥ річного орієнтира, очікують фінального підпису)")
    ...
    if auto_completed_list:
        with st.expander(f"⚡ Авто-зараховані системою заходи: {len(auto_completed_list)} — деталі"):
            st.caption(
                ...
                "підпису система або нічого не змінить, або перерахує за підтвердженими даними."
            )
            st.dataframe(
...
_flex_pct = round(100.0 * _num / _den, 1) if _den else 0.0
_f1, _f2 = st.columns([1, 2.2])
with _f1:
    st.metric("Виконання за обраною базою", f"{_flex_pct}%", f"{_num} із {_den}")
with _f2:
    st.caption(
