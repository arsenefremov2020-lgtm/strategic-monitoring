...
if "chat_channel" not in st.session_state:
    st.session_state.chat_channel = "coordinator"
if "chat_input" not in st.session_state:
    st.session_state.chat_input = ""

# Кнопка-пташка (FAB)
st.markdown("""
<div class="chat-fab" onclick="window.chatFabClick && window.chatFabClick()" id="chatFab"
     title="Чат">
  <!-- Синя пташка SVG -->
...
# Кнопка-тригер для відкриття чату через Streamlit
col_fab, _ = st.columns([1, 8])
with col_fab:
    if st.button("💬 Чат", key="open_chat_btn",
                 help="Відкрити чат з координатором / відповідальним"):
...
if st.session_state.chat_open:
    ...
    selected_channel = st.radio(
        ...
        horizontal=True,
        key="chat_channel_select"
    )
    st.session_state.chat_channel = selected_channel

    # Завантаження повідомлень
    messages = load_chat_messages(st.session_state.chat_channel)

    # Відображення повідомлень
    chat_container = st.container()
    with chat_container:
        if not messages:
            st.caption("Повідомлень поки немає. Напишіть першим 👇")
        else:
            for msg in messages[-30:]:  # Останні 30
                sender = msg.get("sender", "")
                text = msg.get("text", "")
                ts = msg.get("created_at", "")[:16].replace("T", " ")
                is_me = sender == "Керівник ССП"

                avatar_color = "#3b82f6" if is_me else "#16a34a"
                avatar_letter = "К" if is_me else ("⚙" if sender == "Координатор" else "В")
                align = "flex-end" if is_me else "flex-start"
                bubble_bg = "linear-gradient(135deg,#1e40af,#3b82f6)" if is_me else "#fff"
                bubble_color = "#fff" if is_me else "#1e293b"
                bubble_border = "none" if is_me else "1px solid #e2e8f0"
                br1 = "14px 14px 4px 14px" if is_me else "14px 14px 14px 4px"

                st.markdown(f"""
                <div style="display:flex;flex-direction:column;align-items:{align};margin-bottom:10px;">
                    <div style="display:flex;align-items:flex-end;gap:8px;{'flex-direction:row-reverse;' if is_me else ''}">
                        <div style="width:28px;height:28px;border-radius:50%;background:{avatar_color};
                             display:flex;align-items:center;justify-content:center;
                             color:#fff;font-size:12px;font-weight:900;flex-shrink:0;">
                            {avatar_letter}
                        </div>
                        <div style="max-width:72%;padding:9px 13px;border-radius:{br1};
                             background:{bubble_bg};color:{bubble_color};border:{bubble_border};
                    ...
                    <div style="font-size:10px;color:#94a3b8;margin-top:3px;
                         {'text-align:right;' if is_me else ''}">
                        {escape(sender)} · {ts}
                    </div>
                </div>
                """, unsafe_allow_html=True)
    ...
    with chat_col1:
        msg_input = st.text_input(
            ...
            key=f"chat_msg_input_{st.session_state.chat_channel}",
            label_visibility="collapsed"
        )
    with chat_col2:
        send_btn = st.button("➤ Надіслати", key=f"send_chat_{st.session_state.chat_channel}",
    ...
    if send_btn and msg_input.strip():
        try:
            ...
            st.error("Не вдалося надіслати повідомлення. Переконайтесь що таблиця chat_messages існує в Supabase.")
            st.exception(e)
