...
fp1, fp2, fp3 = st.columns([1.25, 1.25, 2.4])
with fp1:
...
if not evaluation_df.empty:
    ...
    weak_goals = add_order_columns(goal_scores, "goal_code").sort_values("goal_score", ascending=True).head(3) if not goal_scores.empty else pd.DataFrame()
    ...
    if not weak_goals.empty:
        ...
        notes_html += insight(f"Найбільшої управлінської уваги потребують стратегічні цілі: <strong>{weak}</strong>.", "warn")
    if delayed_share > 0:
        notes_html += insight(
