...
IMPLEMENTED_MODES = {MODE_MZAHODY}
...
_MIO_STATUS_BG = {
    ST_DONE:     ("#dcfce7", "#166534"),
    ST_PARTIAL:  ("#fef3c7", "#92400e"),
    ST_NOTDONE:  ("#fee2e2", "#991b1b"),
    ST_NOTYET:   ("#f1f5f9", "#475569"),
    ST_OBSOLETE: ("#ede9fe", "#5b21b6"),
}
_MIO_STATUS_COLS = ["Стан · I кв", "Стан · I пів", "Стан · 9 міс", "Стан · РІК"]


def _mio_style_status(val):
    bg, fg = _MIO_STATUS_BG.get(val, ("", ""))
    return f"background-color:{bg};color:{fg};font-weight:700;" if bg else ""


def _mio_style_ratio(val):
    ...
    return "background-color:#f1f5f9;color:#94a3b8;font-weight:700;"  # "х"
...
def _mio_table_styles(df):
    styles = pd.DataFrame("", index=df.index, columns=df.columns)
    for col in _MIO_STATUS_COLS:
        if col in df.columns:
            styles[col] = df[col].map(_mio_style_status)
    if "Факт/План, %" in df.columns:
        styles["Факт/План, %"] = df["Факт/План, %"].map(_mio_style_ratio)
    return styles


def _render_mzahody_year(strat_df, monitoring_df, year):
    df = build_mio_measures_table(strat_df, monitoring_df, year)
    if df.empty:
        st.info("Немає заходів для відображення.")
        return

    # Зведення за станом виконання РІК
    sc = df["Стан · РІК"].value_counts().to_dict()
    n_total    = len(df)
    n_done     = sc.get(ST_DONE, 0)
    n_partial  = sc.get(ST_PARTIAL, 0)
    n_notdone  = sc.get(ST_NOTDONE, 0)
    n_notyet   = sc.get(ST_NOTYET, 0)
    n_obsolete = sc.get(ST_OBSOLETE, 0)

    kpis = "".join([
        kpi_card("Усього заходів", n_total, "gray"),
        kpi_card("Виконано", n_done, "green"),
        kpi_card("Частково виконано", n_partial, "yellow"),
        kpi_card("Не виконано", n_notdone, "red"),
        kpi_card("Не настав час", n_notyet, "blue"),
        kpi_card("Втратило актуальність", n_obsolete, "gray"),
    ])
    st.markdown(
    ...
    styler = df.style.apply(_mio_table_styles, axis=None).format(
        {"Факт/План, %": _mio_fmt_ratio}
    )

    col_cfg = {
        "Стратегічна ціль":       st.column_config.TextColumn("Стратегічна ціль", width="medium"),
        "Завдання":               st.column_config.TextColumn("Завдання", width="medium"),
        "Захід":                  st.column_config.TextColumn("Захід", width="small"),
        "Назва заходу":           st.column_config.TextColumn("Назва заходу", width="large"),
        "Індикатор":              st.column_config.TextColumn("Індикатор", width="large"),
        "Од. виміру":             st.column_config.TextColumn("Од. виміру", width="small"),
        "План (ціль. орієнтир)":  st.column_config.TextColumn("План (цільовий орієнтир)", width="small"),
        "Факт/План, %":           st.column_config.TextColumn("Факт/План, %", width="small"),
    }
