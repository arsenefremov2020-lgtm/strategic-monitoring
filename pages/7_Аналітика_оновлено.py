...
def load_strat_matrix():
    ...
    def find_col_by_keywords(keywords):
        ...
        for col_idx in range(source_df.shape[1]):
            joined = " ".join(raw_value(header_area.iloc[row_idx, col_idx]).lower() for row_idx in range(len(header_area)))
    ...
    # Backward compatibility for files where period columns are only in W/X.
...
def create_excel_report(active, period_requests, goal_progress, dep_progress, task_progress, product_progress, status_counts, period_dynamics, metrics, filters):
    output = BytesIO()
