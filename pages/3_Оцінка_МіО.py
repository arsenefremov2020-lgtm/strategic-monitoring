...
# ============================================================
# ФОРМУЛИ РЕЖИМІВ — «реальна» (як фактично закладено в коді) +
# «спрощена» (простими словами). ВАЖЛИВО: текст нижче звірений
# безпосередньо з робочими функціями розрахунку (mio_fact_plan_ratio,
# mio_year_status, rv_period_score, rv_year_score, rv_final_result,
# rv_averageif, rv_goal_final_result, mio_gt_progress_score,
# mio_gt_change, build_integral_table, build_financing_table) — а НЕ
# переписаний з коментарів-документації над ними. Якщо колись формулу
# в коді буде змінено, цей текст теж потрібно оновити вручну.
# ============================================================

def _formula_row(label, real, simple):
    return (
        f'<div class="formula-row">'
        f'<div class="f-label">{_esc(label)}</div>'
        f'<div class="f-real">{real}</div>'
        f'<div class="f-simple">{simple}</div>'
        f'</div>'
    )


def render_formula_box(title, rows, note=None):
    ...
    for label, real, simple in rows:
        html += _formula_row(label, real, simple)
    if note:
        html += f'<div class="fb-note">{note}</div>'
    html += '</div>'
    st.markdown(html, unsafe_allow_html=True)


FORMULA_MZAHODY = [
    ("1. Співвідношення<br>Факт/План, %",
     "ЯКЩО є стан «Втратило акт.» серед I кв./I пів./9 міс → «в/а»<br>"
     "ІНАКШЕ ЯКЩО План = «х» АБО Факт_рік порожній → «х»<br>"
     "ІНАКШЕ ЯКЩО одиниця = «так/ні» → 100, якщо Факт=План=«так»; інакше 0<br>"
     "ІНАКШЕ → Факт_рік ÷ План_рік × 100",
     "Наскільки фактичне річне значення відповідає тому, що планували "
     "на рік, у відсотках. «в/а» — захід уже неактуальний; «х» — ще "
     "зарано рахувати (немає плану чи факту)."),
    ("2. Стан виконання<br>за РІК",
     "ЯКЩО є «Втратило акт.» серед кварталів → «Втратило акт.»<br>"
     "ІНАКШЕ ЯКЩО План=«х» або Факт_рік порожній → «Не настав час»<br>"
     "ІНАКШЕ (так/ні) → «Виконано» якщо факт=план, «Не виконано» якщо факт=«ні», інакше «Не настав час»<br>"
     "ІНАКШЕ за Формулою 1: &gt;99.99% → «Виконано»; 74.99–100% (не вкл.) → «Частково»; "
     "=0% → «Не настав час»; інакше → «Не виконано»",
     "Підсумковий словесний статус заходу за рік, виведений із того "
     "самого відсоткового співвідношення вище."),
]

FORMULA_RV_MEAS = [
    ("3. Бал за квартал<br>(I кв / I пів / 9 міс)",
     "«Виконано» → 1.0 &nbsp;|&nbsp; «Частково виконано» → 0.75<br>"
     "«Не виконано» → 0.0 &nbsp;|&nbsp; «Втратило акт.» → «в/а» &nbsp;|&nbsp; інше → «х»",
     "Переклад словесного стану кварталу (з режиму «М_заходи») в число "
     "від 0 до 1, щоб можна було рахувати середнє."),
    ("4. Бал за РІК",
     "Співвідношення_Факт/План (формула 1) ÷ 100<br>"
     "(«х»/«в/а» лишаються як є)",
     "Те саме річне співвідношення факту й плану, просто переведене з "
     "відсотків у частку від 0 до 1."),
    ("5. Кінцевий<br>результат",
     "Бал_РІК = «х» → «Не настав час»<br>Бал_РІК = «в/а» → «Втратило акт.»<br>"
     "Бал_РІК ≥ 1.0 → «Виконано»<br>0.75 &lt; Бал_РІК &lt; 1.0 → «Частково виконано»<br>"
     "інакше (вкл. рівно 0.75) → «Не виконано»",
     "Підсумковий стан заходу за рік, порахований уже від числового "
     "бала, а не напряму від співвідношення."),
]

FORMULA_RV_GOALS = [
    ("6. Бал завдання",
     "СЕРЕДНЄ( Бал заходів, чий код починається з коду завдання ) — "
     "рахуються ЛИШЕ числові бали, «х»/«в/а» виключаються з вибірки; "
     "якщо числових немає — «х»",
     "Бал завдання — це середній бал усіх ЙОГО заходів (тексти "
     "«х»/«в/а» просто не враховуються, ні в суму, ні в кількість)."),
    ("7. Бал<br>стратегічної цілі",
     "СЕРЕДНЄ( Бал завдань, чий код починається з коду цілі ) — та сама "
     "логіка виключення «х»/«в/а», що й вище",
     "Бал цілі — це середнє від УЖЕ порахованих балів її завдань (а "
     "НЕ пряме середнє від усіх заходів цілі одразу) — двоступеневий "
     "підйом угору по ієрархії."),
    ("8. Кінцевий<br>результат (ціль/завдання)",
     "Бал_РІК = 1.0 → «Виконано»<br>0.75 &lt; Бал_РІК &lt; 1.0 → «Частково виконано»<br>"
     "Бал_РІК = «х» → «Не настав час»<br>інакше → «Не виконано»",
     "Той самий принцип переведення бала в словесний стан, застосований "
     "уже до бала цілі чи завдання."),
]

FORMULA_MIO_GT = [
    ("9. Зміна до<br>попереднього року, %",
     "«так/ні» → «х»<br>інакше → Факт_рік ÷ Факт_попередній_рік × 100",
     "У скільки разів (у відсотках) фактичне значення цього року "
     "більше чи менше за минулорічне."),
    ("10. Оцінка прогресу —<br>ЗАВДАННЯ",
     "«так/ні» → «Виконується» якщо факт ствердний, інакше «х»<br>"
     "інакше → Факт_рік ÷ Ціль_2028 × 100",
     "Для показників завдань — пряма пропорція: скільки відсотків від "
     "кінцевої мети 2028 року вже досягнуто."),
    ("11. Оцінка прогресу —<br>ЦІЛЬ, крок 1 (база)",
     "База = Факт_2025, якщо є<br>інакше База = Факт_2024<br>"
     "інакше (останній резерв) База = Факт_2021<br>"
     "n1 = Рік_розрахунку − Рік_бази &nbsp;|&nbsp; n2 = 2028 − Рік_бази",
     "Спочатку визначаємо точку відліку (базу) і скільки років рахувати "
     "«вперед» від неї — пріоритет за свіжістю даних: 2025 → 2024 → 2021."),
    ("12. Оцінка прогресу —<br>ЦІЛЬ, крок 2 (проміжні числа)",
     "rc = (Факт_рік ÷ База) ^ (1 ÷ n1)<br>"
     "rt = (Ціль_2028 ÷ База) ÷ База) ^ (1 ÷ n2)",
     "Беремо корінь відповідного степеня від того, наскільки факт "
     "«відірвався» від бази (rc), і окремо — наскільки від бази "
     "«відірвана» ціль 2028 року (rt). Це і є траєкторія: рівномірний "
     "щорічний темп, який привів би від бази точно до цілі."),
    ("13. Оцінка прогресу —<br>ЦІЛЬ, крок 3 (підсумок)",
     "ЯКЩО Ціль_2028 &lt; База (мета — зниження):<br>"
     "&nbsp;&nbsp;Оцінка = (200 − rc×100) ÷ (200 − rt×100) × 100<br>"
     "ІНАКШЕ (мета — зростання):<br>&nbsp;&nbsp;Оцінка = rc ÷ rt × 100",
     "Якщо мета — зменшити показник (наприклад, рівень безробіття) — "
     "числа спершу «дзеркалять» відносно 100, бо там менше = краще. "
     "Якщо мета — зростання, просто ділимо прогрес на потрібний темп."),
]

FORMULA_INTEGRAL = [
    ("14. Інтегральна оцінка<br>цілі за рік",
     "Інтеграл = 0.20 × Заходи_% + 0.30 × Завдання_серед_% + 0.50 × Показники_ціль_серед_%<br>"
     "(відсутній компонент → підставляється 0, вага все одно застосовується)",
     "Три складові з різною вагою складаються в одну підсумкову цифру: "
     "20% — наскільки виконані заходи цілі (з режиму «РВ (СЦ, "
     "Завдання)»), 30% — середній прогрес по завданнях цілі, 50% — "
     "прогрес власних показників самої цілі (обидва — з режиму «МіО "
     "цілі/завдання»)."),
    ("14а. Складова<br>«Завдання» (30%)",
     "СЕРЕДНЄ( СЕРЕДНЄ(Оцінка_прогресу показників завдання) — по "
     "кожному завданню цілі )",
     "Спочатку для кожного завдання рахуємо середню оцінку прогресу "
     "його показників, а потім усереднюємо ці числа між усіма "
     "завданнями цілі."),
]

FORMULA_FINANCING = [
    ("15. % фінансового<br>виконання",
     "Факт_бюджет_млрд ÷ План_бюджет_млрд × 100",
     "Скільки відсотків запланованого бюджету фактично освоєно."),
    ("16. Стан виконання<br>заходу, %",
     "Бал_РІК заходу (з режиму «РВ (Заходи)») × 100",
     "Той самий фізичний бал виконання заходу, просто переведений у "
     "відсотки для порівняння з фінансовим показником поруч."),
    ("17. Коефіцієнт<br>еластичності",
     "% фінансового виконання ÷ Стан виконання заходу, %",
     "≈ 1 — гроші витрачаються пропорційно результату. &gt; 1 — витрачено "
     "більше, ніж досягнуто фізично. &lt; 1 — результат випереджає "
     "освоєння бюджету."),
]
...
def render_mode_mzahody(strat_df, monitoring_df, years):
    ...
    with st.expander("🧮 Формули цього режиму (реальні + простими словами)"):
        render_formula_box("М_заходи — 2 формули", FORMULA_MZAHODY)
    if len(years) == 1:
...
def render_mode_rv_meas(strat_df, monitoring_df, years):
    ...
    with st.expander("🧮 Формули цього режиму (реальні + простими словами)"):
        render_formula_box("РВ (Заходи) — 3 формули", FORMULA_RV_MEAS)
    if len(years) == 1:
...
def render_mode_rv_goals(strat_df, monitoring_df, years):
    ...
    with st.expander("🧮 Формули цього режиму (реальні + простими словами)"):
        render_formula_box("РВ (СЦ, Завдання) — 3 формули", FORMULA_RV_GOALS)
    if len(years) == 1:
...
def render_mode_mio_gt(strat_df, monitoring_df, years):
    ...
    with st.expander("🧮 Формули цього режиму (реальні + простими словами)"):
        render_formula_box(
            "МіО цілі/завдання — 5 формул (найскладніший режим — траєкторія для цілей)",
            FORMULA_MIO_GT,
            note="Формули 11–13 стосуються ЛИШЕ показників стратегічних цілей "
                 "(довжина коду = 2, напр. «1.»). Для показників завдань діє "
                 "проста Формула 10.",
        )
...
def render_mode_integral(strat_df, monitoring_df, years):
    ...
    with st.expander("🧮 Формули цього режиму (реальні + простими словами)"):
        render_formula_box(
            "Інт_Оцінка — 2 формули (зважування вже готових результатів)",
            FORMULA_INTEGRAL,
            note="Відсутній компонент рахується як 0, а не пропускається — "
                 "тобто брак даних по будь-якому з трьох компонентів завжди "
                 "тягне інтеграл вниз.",
        )
...
def render_mode_financing(strat_df, monitoring_df, years):
    ...
    with st.expander("🧮 Формули цього режиму (реальні + простими словами)"):
        render_formula_box("МіО Фінансування — 3 формули", FORMULA_FINANCING)
...
def render_mode_infogr_sc(strat_df, monitoring_df, years):
    ...
    with st.expander("🧮 Формули цього режиму"):
        st.markdown(
            '<div class="formula-box"><div class="fb-title">Інфограф СЦ — 0 нових формул</div>'
            '<div class="fb-note">Цей режим нічого не рахує заново — це виключно '
            'візуальне зведення вже готових чисел: середній «Інтеграл» по цілях '
            '(режим «Інт_Оцінка»), стан виконання заходів (режим «РВ (Заходи)») '
            'і сума фактичного/планового бюджету (режим «МіО Фінансування»). '
            'Захід вважається «активним» обраного року, якщо для нього НЕ стоїть '
            'прочерк «х» у плановому орієнтирі саме цього року.</div></div>',
            unsafe_allow_html=True,
        )
...
def render_mode_infogr_sczz(strat_df, monitoring_df, years):
    ...
    with st.expander("🧮 Формули цього режиму"):
        st.markdown(
            '<div class="formula-box"><div class="fb-title">Інфограф_СЦ.З.з — 0 нових формул</div>'
            '<div class="fb-note">Так само, як і «Інфограф СЦ» — жодних нових обчислень, '
            'лише те саме зведення (Інт_Оцінка, РВ (Заходи), МіО цілі/завдання), '
            'звужене до завдань ОДНІЄЇ обраної вами стратегічної цілі.</div></div>',
            unsafe_allow_html=True,
        )
