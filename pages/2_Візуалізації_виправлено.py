...
with kpi_cols[0]:
    st.metric("Активні заходи", total_active, "у вибраному портфелі")
with kpi_cols[1]:
    st.metric("Виконання", f"{completion}%", "середня оцінка")
with kpi_cols[2]:
    st.metric("Погоджені дані", f"{coverage}%", f"{submitted_count} із {total_active}")
with kpi_cols[3]:
    st.metric("Критичний ризик", critical_count, f"середній + критичний: {critical_count + medium_count}")
with kpi_cols[4]:
    st.metric("Без погоджених даних", without_data, pct_value(without_data, total_active))
