...
def validate_submission():
    ...
    if edited_df is None or (hasattr(edited_df, "empty") and edited_df.empty):
        errors.append("Позначте хоча б один захід для подання")
        return errors

    # В AG Grid вибір рядків через checkboxSelection — беремо рядки де submit == True
    selected_rows = edited_df[edited_df["submit"] == True].copy() if "submit" in edited_df.columns else pd.DataFrame()
...
if submit_clicked:
    ...
    if validation_errors:
        ...
        selected_rows = edited_df[edited_df["submit"] == True].copy()
