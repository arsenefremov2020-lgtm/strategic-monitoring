...
chain_columns_exist = "approval_chain" in _raw_monitoring_df.columns
kind_column_exists = "object_kind" in _raw_monitoring_df.columns
...
def render_scheme_picker(ssp_index, key_prefix):
    ...
    for i, role in enumerate(roles_in_scheme):
        with cols[i]:
            ...
            if not candidates:
                st.warning(f"Ланка «{label}»: у системі немає жодного користувача цієї ролі для ССП {ssp_index}.")
                ready = False
                continue
...
if table_df.empty:
    ...
    col_config = {
        ...
        "Опис\nпрогресу": st.column_config.TextColumn(
            "🟡 Опис\nпрогресу",
            help="Необов'язкове поле. Коротко опишіть фактичний прогрес за звітний квартал",
            width=300,
        ),
        "Ризики / проблеми /\nвідхилення": st.column_config.TextColumn(
            "🟡 Ризики / проблеми /\nвідхилення",
    ...
    st.markdown(
        ...
        <div class="note-box" style="background:#f8fafc;border:1px solid #d8dee9;">
            ...
            «{quarter_label}» (квартальне значення), «Статус виконання» — без них подання не пройде
            <span style="background:#fef6e0;border:1px solid #fde68a;border-radius:8px;
                  padding:2px 10px;margin:0 6px;font-weight:800;color:#92400e;">🟡 Необов'язкові</span>
            «Опис прогресу», «Ризики / проблеми / відхилення», «Посилання на НПА»
        </div>
        """,
        unsafe_allow_html=True,
    )
