...
# ============================================================
# Stream 1 (Excel): indicators progress for goals & tasks
#   МіО_цілі_завдан  → fact values + pre-computed progress scores
#   РВ (СЦ, Завд.)_РОЗРАХ → aggregated measure execution scores
# Stream 2 (Supabase): measures execution data (approved submissions)
# ============================================================

MIO_SHEET      = "МіО_цілі_завдан"
MEASURES_SHEET = "РВ (СЦ, Завд.)_РОЗРАХ"

# МіО_цілі_завдан column positions (1-indexed, verified from Excel):
YEAR_COLS = {
...
def load_mio_excel():
    """
    STREAM 1a — Excel МіО_цілі_завдан.
...
with st.expander("🔧 Панель адміністратора — деталізація всіх розрахунків", expanded=False):
    ...
    st.markdown(
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:14px;">
          <div class="section-card" style="margin:0;">
            <div class="section-title">📗 Stream 1a — Excel МіО_цілі_завдан</div>
            <div class="methodology-box">
              <strong>Аркуш:</strong> МіО_цілі_завдан<br>
              ...
              <strong>Колонки факт:</strong> J(2026) N(2027) R(2028)<br>
              <strong>Колонки прогресу:</strong> M(2026) Q(2027) W(2028)<br>
    ...
    if not evaluation_df.empty and not task_scores.empty and not goal_scores.empty:
        ...
        agg_rows.append({
            "Рівень": f"Прогрес індикаторів (Stream 1: Excel МіО) — {excel_goals}/{total_goals} цілей мають дані",
            "Кількість": total_goals,
            "Середня оцінка, %": w_goals_val,
            "Вага у формулі цілі": "50%",
            "Внесок в інтегральну": f"{round(w_goals_val * 0.50, 2)}%",
            "Формула": "MEAN прогресу індикаторів з Excel МіО_цілі_завдан (col M/Q/W)"
        })
        agg_rows.append({
        ...
        if goal_sources:
            ...
            for g_code, src in goal_sources.items():
                gs_rows.append({
                    ...
                    "Прогрес K (Excel МіО), %": src["indicator_progress"] if src["indicator_progress"] is not None else "н.д.",
