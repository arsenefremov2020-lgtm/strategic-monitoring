...
def _mio_indicator_year_fact(mon_index, code, year):
    ...
    """
    rec = mon_index.get((raw_value(code), str(year), "IV"))
    if rec is None:
        return ""
...
#   • Бюджетні поля (КПКВК, інше джерело, План/Факт у млрд грн) —
#     з ОКРЕМОГО Excel «БП під моніторинг СП.xlsx» (FIN_FILE_PATH).
#     Файл вноситься окремо; поки його немає — режим деградує:
#     показує заходи й стан виконання, а бюджетні колонки — прочерками
#     з підказкою про очікувану структуру файлу.
#
...
# Очікувана структура «БП під моніторинг СП.xlsx».
# Підтримуються ДВА формати (визначається автоматично за заголовками):
#
#  A) ДОВГИЙ (по рядку на захід+рік):
#     Код заходу | Рік | КПКВК | Інше джерело фінансування |
#     План (млрд грн) | Факт (млрд грн)
#
#  B) ШИРОКИЙ (по рядку на захід, роки в колонках):
#     Код заходу | КПКВК | Інше джерело фінансування |
#     План 2026 | Факт 2026 | План 2027 | Факт 2027 | План 2028 | Факт 2028
#
...
def build_financing_table(strat_df, monitoring_df, fin_index, year):
    ...
    for _, r in base.iterrows():
        code = r["Захід"]
        fin = _fin_lookup(fin_index, code, year)

        kpkvk    = fin.get("kpkvk", "")
        other_src = fin.get("other_source", "")
...
def _render_financing_year(strat_df, monitoring_df, fin_index, year):
    ...
    has_fin = df_full[["План, млрд грн", "Факт, млрд грн"]].notna().any().any()
    ...
    if not has_fin:
        st.warning(
            "⚠️ Бюджетних даних не знайдено. Режим показує перелік заходів і "
            "«Стан виконання заходу, %», а бюджетні колонки — прочерками. "
            "Щоб з'явилися КПКВК / План / Факт у млрд грн, додайте файл "
            "**«БП під моніторинг СП.xlsx»** поряд із застосунком. Очікувані колонки "
            "(довгий формат): `Код заходу`, `Рік`, `КПКВК`, `Інше джерело фінансування`, "
            "`План (млрд грн)`, `Факт (млрд грн)`; або широкий формат: `Код заходу`, "
            "`КПКВК`, `Інше джерело фінансування`, `План 2026`, `Факт 2026`, "
            "`План 2027`, `Факт 2027`, `План 2028`, `Факт 2028`."
        )
...
def render_mode_financing(strat_df, monitoring_df, years):
    ...
    st.markdown(
        ...
        "Бюджетні дані беруться з окремого файлу <b>«БП під моніторинг СП.xlsx»</b>."
        "</div>",
        unsafe_allow_html=True
    )
