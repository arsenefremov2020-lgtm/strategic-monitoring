...
DEFAULT_ACTION_EXECUTOR_COLUMNS = [
    # Людські назви колонок
    "Головний виконавець",
    "Співвиконавець 1",
    "Співвиконавець",

    # Назви з переносами, які можуть бути у таблицях Streamlit
    "Головний\nвиконавець",
    "Співвиконавець\n1",

    # Внутрішні назви колонок у load_strat_matrix()
    "resp_main",
    "resp_co_1",
]
...
def should_lock_ssp_fields(user: dict | None) -> bool:
    ...
    Логіка:
    - ССП: заблоковано, бо індекс підтягнутий автоматично;
    - керівник ССП: заблоковано, бо індекс підтягнутий автоматично;
    - адмін: не заблоковано, але список має бути обмежений його закріпленими ССП;
    - супер-адмін: не заблоковано, бачить усі ССП;
    - гість: заблоковано.
    """
    ...
    if role in [ROLE_SSP, ROLE_SSP_HEAD, ROLE_UNIT_HEAD, ROLE_SSP_DEPUTY, ROLE_GUEST]:
        return True
...
def should_prefill_contact_fields(user: dict | None) -> bool:
    ...
    role = get_user_role(user)
    return role in [ROLE_SSP, ROLE_SSP_HEAD, ROLE_UNIT_HEAD, ROLE_SSP_DEPUTY, ROLE_ADMIN, ROLE_SUPER_ADMIN]
