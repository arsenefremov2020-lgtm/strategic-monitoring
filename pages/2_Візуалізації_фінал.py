...
if not presentation_mode and view_mode in ["Усі візуалізації", "Ризики"]:
    ...
    with rc1:
        st.markdown('<div class="chart-wrap">', unsafe_allow_html=True)
        st.markdown('<div class="chart-title">Розподіл заходів за рівнем ризику</div>', unsafe_allow_html=True)
        fig_risk = px.pie(
            risk_counts,
            names="auto_risk",
            values="Кількість",
    ...
    with rc2:
        st.markdown('<div class="chart-wrap">', unsafe_allow_html=True)
        st.markdown('<div class="chart-title">ТОП-20 заходів із найвищим ризиком</div>', unsafe_allow_html=True)

        risk_top = active[active["auto_risk"] != "Не оцінюється"].sort_values("risk_score", ascending=False).head(20).copy()
        risk_top["label"] = risk_top["code"].astype(str) + " " + risk_top["name"].astype(str).str[:45]

        fig_risk_top = px.bar(
            risk_top.sort_values("risk_score", ascending=True),
            x="risk_score",
            y="label",
            orientation="h",
            text=risk_top.sort_values("risk_score", ascending=True)["risk_score"].apply(lambda x: f"{x:.0f}"),
            color="auto_risk",
            color_discrete_map=RISK_COLORS,
            hover_data={"risk_reason": True, "status_display": True, "period_label": True}
        )
        fig_risk_top.update_layout(
            **CHART_LAYOUT,
            height=max(350, len(risk_top) * 28 + 40),
            xaxis=dict(range=[0, 110], showgrid=True, gridcolor="#f1f5f9"),
            yaxis=dict(showgrid=False),
            margin=dict(l=10, r=20, t=10, b=10)
        )
        fig_risk_top.update_traces(textposition="outside", textfont_size=10)
...
if not presentation_mode and view_mode in ["Усі візуалізації", "Динаміка"]:
    st.markdown('<div class="section-card">', unsafe_allow_html=True)
    st.markdown('<div class="section-title">Динаміка виконання за періодами</div>', unsafe_allow_html=True)

    period_summary = (
        active_period_rows
        .groupby(["period_year", "period_quarter", "period_label"])
        .agg(
            Виконання=("performance_score", "mean"),
            Покриття=("status", lambda x: (x != "Не подано").sum() / len(x) * 100),
            Ризик=("risk_score", "mean"),
            Активних=("code", "count")
        )
        .reset_index()
    )
    period_summary["sort"] = period_summary["period_year"].astype(int) * 10 + period_summary["period_quarter"].apply(quarter_to_number)
    period_summary = period_summary.sort_values("sort")
    period_summary["Виконання"] = period_summary["Виконання"].fillna(0).round(1)
    period_summary["Покриття"] = period_summary["Покриття"].fillna(0).round(1)
    period_summary["Ризик"] = period_summary["Ризик"].fillna(0).round(1)

    st.markdown('<div class="chart-wrap">', unsafe_allow_html=True)
    st.markdown('<div class="chart-title">Тренд виконання, покриття та ризику</div>', unsafe_allow_html=True)

    fig_dyn = go.Figure()
    fig_dyn.add_trace(go.Scatter(
        x=period_summary["period_label"], y=period_summary["Виконання"],
        mode="lines+markers+text", name="Виконання, %",
        text=period_summary["Виконання"].apply(lambda x: f"{x:.1f}%"),
        textposition="top center",
        line=dict(color="#005BBB", width=3), marker=dict(size=8)
    ))
    fig_dyn.add_trace(go.Scatter(
        x=period_summary["period_label"], y=period_summary["Покриття"],
        mode="lines+markers", name="Покриття, %",
        line=dict(color="#0891b2", width=3, dash="dash"), marker=dict(size=7)
    ))
    fig_dyn.add_trace(go.Scatter(
        x=period_summary["period_label"], y=period_summary["Ризик"],
        mode="lines+markers", name="Середній ризик",
        line=dict(color="#dc2626", width=3, dash="dot"), marker=dict(size=7)
    ))
    fig_dyn.update_layout(
        **CHART_LAYOUT,
        height=360,
        xaxis=dict(showgrid=False),
        yaxis=dict(range=[0, 110], showgrid=True, gridcolor="#f1f5f9", ticksuffix="%"),
...
if not presentation_mode and view_mode in ["Усі візуалізації", "Heatmap"]:
    st.markdown('<div class="section-card">', unsafe_allow_html=True)
    st.markdown('<div class="section-title">Heatmap: стратегічні цілі × ССП</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-subtitle">Колір показує середній рівень виконання у відповідному перетині</div>', unsafe_allow_html=True)

    heat_data = explode_departments(active)
    heat = (
        heat_data
        .groupby(["goal_code", "ssp_department"])
        .agg(Виконання=("performance_score", "mean"), Заходів=("code", "count"))
        .reset_index()
    )
    heat["Виконання"] = heat["Виконання"].fillna(0).round(1)

    if not heat.empty:
        heat_pivot = heat.pivot(index="goal_code", columns="ssp_department", values="Виконання").fillna(0)
        ...
        fig_heat = px.imshow(
            ...
            zmin=0,
            zmax=100,
            labels=dict(color="Виконання, %")
        )
        fig_heat.update_layout(
            **CHART_LAYOUT,
            height=max(350, len(heat_pivot) * 45 + 100),
        ...
        st.info("Недостатньо даних для побудови heatmap.")
...
if not presentation_mode and view_mode in ["Усі візуалізації", "Таблиці"]:
    st.markdown('<div class="section-card">', unsafe_allow_html=True)
    st.markdown('<div class="section-title">Аналітичні таблиці</div>', unsafe_allow_html=True)
    ...
    with tab1:
        display_goal = goal_progress[[
            "goal_code", "strategic_goal", "Активних_заходів", "Виконання",
            "Покриття_%", "Ризикових", "Середній_ризик"
        ]].rename(columns={
            "goal_code": "Код СЦ",
            ...
            "Виконання": "Виконання, %",
            "Покриття_%": "Покриття, %",
            "Середній_ризик": "Середній ризик"
        })
        st.dataframe(display_goal, use_container_width=True, hide_index=True)

    with tab2:
        display_dep = dep_progress.rename(columns={
            "ssp_department": "Самостійний структурний підрозділ",
            "Активних_заходів": "Активних заходів",
            "Виконання": "Виконання, %",
            "Покриття_%": "Покриття, %",
    ...
    with tab3:
        detail_cols = [
            "period_label", "goal_code", "task_code", "code", "name",
            "department", "department_co_1", "department_co_2",
            "product_type", "status_display", "performance_score",
            "auto_risk", "risk_score", "risk_reason", "numeric_value", "selected_target"
        ]
        display_detail = active[detail_cols].rename(columns={
            "period_label": "Період",
            "goal_code": "СЦ",
            "task_code": "Завдання",
            "code": "Код заходу",
            "name": "Захід",
            "department": "Відповідальний ССП",
            "department_co_1": "Співвідповідальний 1",
            "department_co_2": "Співвідповідальний 2",
            "product_type": "Тип продукту",
            "status_display": "Статус",
            "performance_score": "Оцінка виконання",
            "auto_risk": "Авто-ризик",
            "risk_score": "Бал ризику",
            "risk_reason": "Пояснення ризику",
            "numeric_value": "Факт",
            "selected_target": "План"
        })
...
if not presentation_mode:
    st.markdown('<div class="section-card">', unsafe_allow_html=True)
    st.markdown('<div class="section-title">Методологія розрахунку</div>', unsafe_allow_html=True)
    st.markdown("""
    <div class="methodology-box">
        <b>1. Активні заходи</b> — заходи, строк реалізації яких перетинається з обраним роком і кварталом.<br>
        <b>2. Виконання СП</b> — середній рівень виконання активних заходів, де кількісний план/факт має пріоритет над статусом.<br>
        <b>3. Покриття моніторингом</b> — частка активних заходів, за якими подано та погоджено моніторингові дані.<br>
        <b>4. Автоматичний ризик</b> — розраховується на основі статусу, відхилення план/факт і прострочення строків.<br>
        <b>5. Заходи зі статусами «Термін не настав» і «Втратив актуальність»</b> не включаються до ризикової оцінки.
    </div>
    """, unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
...
st.markdown(f"""
<div class="footer">
    Дані стратегічної матриці та погоджених заявок Supabase · Оновлено {datetime.now().strftime("%d.%m.%Y %H:%M")}
</div>
""", unsafe_allow_html=True)
