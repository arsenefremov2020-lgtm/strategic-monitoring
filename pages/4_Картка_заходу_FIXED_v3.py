...
def load_strat_matrix():
    ...
    def header_text(col_idx, header_rows=12):
        max_rows = min(header_rows, source_df.shape[0])
        return " ".join(
            clean(source_df.iloc[row_idx, col_idx]).lower()
            for row_idx in range(max_rows)
        )

    def find_col_by_keywords(keywords, forbidden=None, header_rows=12):
        """
        Шукає колонку за текстом у верхніх рядках матриці.
        Для Початку/Кінця виконання використовуємо не просто 'початок'/'кінець',
        а комбінацію з 'виконання', щоб не підхопити випадкові квартальні
        або фінансові колонки.
        """
        keywords = [k.lower() for k in keywords]
        forbidden = [f.lower() for f in (forbidden or [])]
        for col_idx in range(source_df.shape[1]):
            joined = header_text(col_idx, header_rows=header_rows)
            if all(keyword in joined for keyword in keywords) and not any(f in joined for f in forbidden):
                return col_idx
        return None
    ...
    start_col = find_col_by_keywords(["початок", "виконання"])
    end_col = find_col_by_keywords(["кінець", "виконання"])
    kpkvk_col = find_col_by_keywords(["кпквк"])
    ...
    result = pd.DataFrame({
        ...
        "start_period": safe_col(data, start_col) if start_col is not None else safe_col(data, 22),
        "end_period": safe_col(data, end_col) if end_col is not None else safe_col(data, 23),

        "kpkvk_code_raw": safe_col(data, kpkvk_col) if kpkvk_col is not None else pd.Series([""] * len(data), index=data.index),
...
n1, spacer, n2 = st.columns([1, 1.5, 1], gap="medium")
