...
def extract_ssp_index(value):
    """Витягує перший числовий індекс ССП з рядка."""
    text = raw_value(value)
    if not text:
        return ""
...
def get_deputy_minister_for_ssp(ssp_index):
    """Повертає заступника Міністра за індексом ССП."""
...
all_ssp_indices = sorted(
    {
        ...
        for field in ("resp_main", "resp_co_1")
        for index in split_ssp_values(row.get(field, ""))
    },
...
st.markdown(
    ...
    <div class="filter-box">
        ...
        <div class="filter-legend">
            Оберіть індекс самостійного структурного підрозділу та звітний період. 
            Система автоматично відобразить заходи, за якими ССП є <strong>головним виконавцем або співвиконавцем 1</strong>. 
            Для заходів де ССП є співвиконавцем 1, графа квартального факту також доступна для заповнення.
        </div>
        <div class="info-grid">
            <div class="info-card">
...
if selected_ssp_index:
    def row_matches_ssp(row):
        # Головний виконавець
        if value_contains_ssp(row.get("resp_main", ""), selected_ssp_index):
            return True
        # Співвиконавець 1 — теж отримує доступ
        if value_contains_ssp(row.get("resp_co_1", ""), selected_ssp_index):
            return True
        return False

    filtered_measures = filtered_measures[
        filtered_measures.apply(row_matches_ssp, axis=1)
    ]
...
# Коди заходів, які вже подано у цьому кварталі для цього ССП
already_submitted_codes = set()
if selected_ssp_index and not monitoring_df.empty:
    submitted_mask = (
        (monitoring_df["department"].astype(str).str.strip() == str(selected_ssp_index).strip())
        & (monitoring_df["year"].astype(str).str.strip() == str(selected_year))
        & (monitoring_df["quarter"].astype(str).str.strip() == str(selected_quarter))
    )
    already_submitted_codes = set(
        monitoring_df.loc[submitted_mask, "strat_code"].astype(str).str.strip().tolist()
    )

# Будуємо DataFrame для AG Grid (плоский, без \n у назвах)
table_rows = []

for _, row in filtered_measures.iterrows():
    code = raw_value(row.get("code", ""))

    is_main_exec = value_contains_ssp(row.get("resp_main", ""), selected_ssp_index) if selected_ssp_index else True
    is_co_exec_1 = value_contains_ssp(row.get("resp_co_1", ""), selected_ssp_index) if selected_ssp_index else False

    role_label = ""
    if is_main_exec and is_co_exec_1:
        role_label = "Головний / Співвик.1"
    elif is_co_exec_1:
        role_label = "Співвиконавець 1"
    elif is_main_exec:
        role_label = "Головний виконавець"

    deputy = get_deputy_minister_for_ssp(extract_ssp_index(row.get("resp_main", "")))
    is_already_submitted = code in already_submitted_codes
    ...
    table_rows.append({
        ...
        "base_2021":      raw_value(row.get("base_2021", "")),
        "fact_2024":      raw_value(row.get("fact_2024", "")),
        "fact_2025":      raw_value(row.get("fact_2025", "")),
        "target_year":    raw_value(row.get(target_col, "")),
        "quarter_fact":   "",
        "source_global":  raw_value(row.get("source_global", "")),
        ...
        "period_years":   raw_value(row.get("measure_period_years", "")),
        "start_date":     raw_value(row.get("measure_start_date", "")),
        "end_date":       raw_value(row.get("measure_end_date", "")),
        "deputy":         deputy,
        "role_ssp":       role_label,
        "sys_status":     "✅ Подано" if is_already_submitted else "",
        "exec_status":    "",
        "progress":       "",
        "risks":          "",
    })
...
# Банер вже поданих
if already_submitted_codes and not table_df.empty:
    already_in_table = [c for c in already_submitted_codes if c in table_df["code"].values]
    if already_in_table:
        st.markdown(
            f"""
            <div class="note-box" style="background:#fef9c3;border:1px solid #fde047;color:#713f12;">
                ⚠️ <strong>Увага:</strong> За {len(already_in_table)} заходом(заходами) відомості
                у {quarter_label} вже подано (позначено ✅ у колонці «Статус у системі»).
                Повторне подання за тим самим заходом і кварталом <strong>заблоковано</strong>.
            </div>
            """,
            unsafe_allow_html=True
        )
...
if table_df.empty:
    ...
    # ----------------------------------------------------------------
    # AG Grid column definitions з grouped headers
    # ----------------------------------------------------------------
    col_defs = [
        {
            "headerName": "Подати",
        ...
        {
            "field": "unit",
            "headerName": "Одиниці виміру",
            "width": 120,
            "editable": False,
        },
        # Річні показники — grouped
        {
            "headerName": "Показники по роках",
            "children": [
                {
                    "field": "base_2021",
                    "headerName": "2021 базовий рівень (факт)",
                ...
                {
                    "field": "target_year",
                    "headerName": f"{selected_year} цільовий орієнтир",
                    "width": 140,
                    "editable": False,
                    "cellStyle": {"textAlign": "center"},
                },
                {
                    "field": "quarter_fact",
                    "headerName": f"{quarter_label} (факт)",
                    "width": 140,
                    "editable": True,
                    "cellStyle": {"backgroundColor": "#fffde7", "textAlign": "center"},
                    "headerClass": "ag-header-editable",
                },
            ],
        },
        # Підстава для включення — grouped
        {
            "headerName": "Підстава для включення до стратегічного плану",
            "headerStyle": {"backgroundColor": "#e3f0e8", "color": "#0F6E56"},
            "children": [
                {
                    "field": "source_global",
                    "headerName": "Глобальний рівень",
                    "width": 260,
                    "editable": False,
                    "wrapText": True,
                    "autoHeight": True,
                ...
                {
                    "field": "source_national",
                    "headerName": "Національний рівень",
                    "width": 260,
                    "editable": False,
                    "wrapText": True,
                    "autoHeight": True,
        ...
        # Відповідальні ССП — grouped
        {
            "headerName": "Відповідальні самостійні структурні підрозділи",
            "children": [
                {
                    "field": "resp_main",
                    "headerName": "Головний виконавець",
                    "width": 180,
                    "editable": False,
                },
                {
                    "field": "resp_co_1",
                    "headerName": "Співвиконавець 1",
                    "width": 160,
                    "editable": False,
                },
                {
                    "field": "resp_co_2",
                    "headerName": "Співвиконавець 2",
                    "width": 160,
                    "editable": False,
                },
            ],
        ...
        {
            "field": "deputy",
            "headerName": "Заступник Міністра",
            "width": 220,
            "editable": False,
            "wrapText": True,
            "autoHeight": True,
            "cellStyle": {"whiteSpace": "normal", "lineHeight": "1.4", "fontStyle": "italic"},
        },
        {
            "field": "role_ssp",
            "headerName": "Роль ССП у заході",
            "width": 160,
            "editable": False,
        ...
        {
            "field": "sys_status",
            "headerName": "Статус у системі",
            "width": 130,
            "editable": False,
            "cellStyle": {"textAlign": "center"},
        },
        # Редаговані поля подання
        {
            "field": "exec_status",
            "headerName": "Статус виконання",
            "width": 180,
            "editable": True,
        ...
        },
        {
            ...
            "cellStyle": {"backgroundColor": "#fffde7", "whiteSpace": "normal", "lineHeight": "1.4"},
            "headerClass": "ag-header-editable",
        },
        {
            "field": "risks",
            "headerName": "Ризики / проблеми / відхилення",
            "width": 300,
            "editable": True,
            "wrapText": True,
            "autoHeight": True,
    ...
    # Стиль рядків — вже подані виділяємо зеленуватим
    row_style_jscode = JsCode("""
    function(params) {
        if (params.data.sys_status && params.data.sys_status.includes('Подано')) {
            return { background: '#f0fdf4', color: '#6b7280' };
        }
        return null;
    }
    """)

    is_row_selectable_jscode = JsCode("""
    function(params) {
        return !(params.data.sys_status && params.data.sys_status.includes('Подано'));
    }
    """)
    ...
    go = gb.build()
    go["columnDefs"] = col_defs
    go["rowSelection"] = "multiple"
    go["suppressRowClickSelection"] = True
    go["isRowSelectable"] = is_row_selectable_jscode
    go["getRowStyle"] = row_style_jscode
    go["headerHeight"] = 48
    go["groupHeaderHeight"] = 36
    go["rowHeight"] = 72
    go["defaultColDef"] = {
        "resizable": True,
        "sortable": False,
    ...
    custom_css = {
        ...
        ".ag-header-row:first-child .ag-header-group-cell": {
        ...
        ".ag-row-even": {"background-color": "#fafcff"},
        ".ag-row-odd":  {"background-color": "#ffffff"},
        ".ag-row-hover":{"background-color": "#eff6ff !important"},
    }

    grid_height = min(900, max(300, 90 + len(table_df) * 75))
    ...
    grid_response = AgGrid(
        ...
        key=f"aggrid_{selected_ssp_index}_{selected_year}_{selected_quarter}_{search_query}",
    )

    edited_df = grid_response["data"] if grid_response and grid_response["data"] is not None else pd.DataFrame()
    # selected_rows — рядки позначені чекбоксом (правильний механізм AG Grid)
    aggrid_selected = pd.DataFrame(grid_response["selected_rows"]) if grid_response and grid_response["selected_rows"] is not None and len(grid_response["selected_rows"]) > 0 else pd.DataFrame()
...
def validate_submission():
    ...
    if aggrid_selected is None or (hasattr(aggrid_selected, "empty") and aggrid_selected.empty):
        errors.append("Позначте хоча б один захід для подання")
        return errors

    selected_rows = aggrid_selected.copy()
    ...
    for _, row in selected_rows.iterrows():
        ...
        if code in already_submitted_codes:
            errors.append(
                f"Захід {code}: відомості за {quarter_label} вже подано. "
                f"Повторне подання неможливе. Зніміть позначку для цього заходу."
            )
            continue

        required_values = [
            row.get("quarter_fact", ""),
            row.get("exec_status", ""),
            row.get("progress", ""),
            row.get("risks", ""),
        ]
    ...
    if validation_errors:
        ...
        for _, row in selected_rows.iterrows():
            ...
            payload.append({
                ...
                "status":             raw_value(row.get("exec_status", "")),
                "strat_code":         code,
                "responsible_person": raw_value(responsible_person),
                "phone":              raw_value(responsible_phone),
                "email":              raw_value(responsible_email),
                "numeric_value":      raw_value(row.get("quarter_fact", "")),
                "progress_text":      raw_value(row.get("progress", "")),
                "risks":              raw_value(row.get("risks", "")),
                "file_names":         "",
                "file_urls":          "",
                "admin_comment":      "",
                "start_date":         raw_value(row.get("start_date", "")),
                "end_date":           raw_value(row.get("end_date", "")),
