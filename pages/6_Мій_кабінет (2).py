...
# Це працює бо Streamlit рендерить st.markdown прямо в основний документ,
# тому position:fixed справді фіксується відносно viewport, а не iframe.
st.markdown(f"""
<style>
/* ── FAB ── */
#bird-fab {{
  position: fixed;
  bottom: 90px;        /* вище панелі Manage app (~72px) */
  right: 28px;
...
#bird-chat-close {{
  cursor:pointer; font-size:18px; opacity:.75;
  padding:2px 5px; border-radius:4px;
}}
#bird-chat-close:hover {{ opacity:1; background:rgba(255,255,255,.18); }}
...
.bmsg-wrap {{ display:flex; flex-direction:column; }}
.bmsg-wrap.me {{ align-items:flex-end; }}
.bmsg-wrap.other {{ align-items:flex-start; }}
.bmsg-row {{ display:flex; align-items:flex-end; gap:7px; }}
.bmsg-wrap.me .bmsg-row {{ flex-direction:row-reverse; }}
.bavatar {{
...
.bmsg-wrap.me .bbubble {{
  background:linear-gradient(135deg,#1e40af,#3b82f6);
  color:#fff; border-bottom-right-radius:4px;
}}
.bmsg-wrap.other .bbubble {{
  background:#fff; border:1px solid #e2e8f0;
  color:#1e293b; border-bottom-left-radius:4px;
  box-shadow:0 1px 4px rgba(0,0,0,.06);
}}
.bmsg-meta {{ font-size:10px; color:#94a3b8; margin-top:3px; }}
.bmsg-wrap.me .bmsg-meta {{ text-align:right; }}
.bempty {{ color:#94a3b8; font-size:13px; text-align:center; margin:auto; padding:20px; }}
...
#bird-input {{
  ...
  background:#f0f9ff; font-family:'Helvetica Neue',Arial,sans-serif;
}}
...
#bird-send:hover {{ filter:brightness(1.1); }}
</style>

<!-- FAB — пташка -->
<div id="bird-fab" onclick="birdToggle()" title="Чат">
...
<div id="bird-chat">
  <div id="bird-chat-header">
    <span>🐦 Чат</span>
    <span id="bird-chat-close" onclick="birdToggle()">✕</span>
  </div>
  <div id="bird-tabs">
    <div class="bctab active" onclick="birdTab('coordinator')" id="btab-coordinator">📋 Координатор</div>
    <div class="bctab"        onclick="birdTab('responsible')"  id="btab-responsible">👤 ССП</div>
    <div class="bctab"        onclick="birdTab('group')"        id="btab-group">👥 Груп</div>
  </div>
  <div id="bird-messages"></div>
  <div id="bird-input-area">
    <input id="bird-input" type="text" placeholder="Напишіть повідомлення..."
           onkeydown="if(event.key==='Enter')birdSend()"/>
    <button id="bird-send" onclick="birdSend()">➤</button>
  </div>
</div>

<script>
(function() {{
  const SB_URL = "{_SUPABASE_URL}";
  const SB_KEY = "{_SUPABASE_KEY}";
  const ME = "Керівник ССП";
  const AV_COLOR = {{"Керівник ССП":"#3b82f6","Координатор":"#16a34a"}};
  const AV_LETTER= {{"Керівник ССП":"К","Координатор":"⚙"}};
  ...
  let curCh = "coordinator";
  let polling = null;
  let cache = JSON.parse(JSON.stringify(INIT));  // deep copy
  ...
  window.birdToggle = function() {{
    const win = document.getElementById("bird-chat");
    const isOpen = win.classList.toggle("open");
    if (isOpen) {{
      render(cache[curCh]);
      scrollBot();
      startPoll();
      setTimeout(()=>document.getElementById("bird-input").focus(), 120);
    }} else {{
      stopPoll();
    }}
  }};

  window.birdTab = function(ch) {{
    curCh = ch;
    document.querySelectorAll(".bctab").forEach(t=>t.classList.remove("active"));
    document.getElementById("btab-"+ch).classList.add("active");
    render(cache[ch]);
    scrollBot();
    fetch_msgs();
  }};
  ...
  function render(msgs) {{
    const box = document.getElementById("bird-messages");
    if (!msgs || !msgs.length) {{
      box.innerHTML = '<div class="bempty">Повідомлень поки немає.<br>Напишіть першим 👇</div>';
      return;
    }}
    box.innerHTML = msgs.slice(-60).map(m => {{
      const isMe = m.sender === ME;
      const col  = AV_COLOR[m.sender]  || "#d97706";
      const lett = AV_LETTER[m.sender] || "В";
      return `<div class="bmsg-wrap ${{isMe?'me':'other'}}">
        <div class="bmsg-row">
          <div class="bavatar" style="background:${{col}}">${{esc(lett)}}</div>
          <div class="bbubble">${{esc(m.text)}}</div>
        </div>
        <div class="bmsg-meta">${{esc(m.sender)}} · ${{esc(m.ts)}}</div>
      </div>`;
    }}).join("");
  }}

  function scrollBot() {{
    const b = document.getElementById("bird-messages");
  ...
  async function fetch_msgs() {{
    try {{
      const r = await fetch(
        SB_URL+"/rest/v1/chat_messages?channel=eq."+encodeURIComponent(curCh)+
        "&order=created_at.asc&limit=80",
        {{headers:{{"apikey":SB_KEY,"Authorization":"Bearer "+SB_KEY}}}}
      );
      if(!r.ok) return;
      const data = await r.json();
      const msgs = data.map(m=>({{"sender":m.sender,"text":m.text,
  ...
  window.birdSend = async function() {{
    const inp = document.getElementById("bird-input");
  ...
  function startPoll(){{
    stopPoll();
    polling = setInterval(fetch_msgs, 8000);
  }}
  function stopPoll(){{
    if(polling){{clearInterval(polling);polling=null;}}
  }}

  // Перший рендер одразу
  render(cache[curCh]);
}})();
</script>
""", unsafe_allow_html=True)
