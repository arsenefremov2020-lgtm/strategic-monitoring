...
st.markdown(
    f"""
<div class="hero">
    <div class="hero-title">Аналітичний дашборд виконання стратегічного плану</div>
    <div class="hero-subtitle">
        Сторінка показує не просто набір графіків, а управлінську картину: що виконано, де немає погоджених даних,
        які цілі та підрозділи формують найбільший ризик і які заходи потребують першочергової уваги.
    </div>
    <div class="pill-row">
        <div class="pill">Джерело: Excel + Supabase</div>
        <div class="pill">Факт: погоджені заявки</div>
        <div class="pill">Оновлено: {last_update}</div>
        <div class="pill">Версія: DEMO 1.5</div>
    </div>
</div>
""",
    unsafe_allow_html=True
...
status_options = ["Виконано", "Частково виконано", "Не виконано", "Термін не настав", "Втратив актуальність", "Виконується"]
...
<div class="filter-shell">
    <div class="filter-header">
        <div>
            <div class="filter-title">Фільтри аналізу</div>
            <div class="filter-hint">Спочатку оберіть період і фокус аналізу. Детальні фільтри відкривайте лише за потреби.</div>
        </div>
    </div>
</div>
""",
...
st.markdown(
    f"""
<div class="panel">
    <div class="panel-title">Висновок системи</div>
    <div class="panel-subtitle">Оцінка формується за останнім доступним періодом для кожного відібраного заходу.</div>
    <div class="pill-row">
        <div class="pill {conclusion_badge}">{conclusion_title}</div>
        <div class="pill">Виконання: {completion}%</div>
        <div class="pill">Погоджені дані: {coverage}%</div>
        <div class="pill">Ризикові заходи: {risk_share}%</div>
        <div class="pill">Відхилення: {deviation_current} в.п.</div>
    </div>
...
st.markdown(
    '<div class="kpi-grid">'
    + kpi_card("Активні заходи", total_active, "у вибраному портфелі", "kpi-blue")
    + kpi_card("Виконання", f"{completion}%", "середня оцінка за заходами", "kpi-green" if completion >= 75 else "kpi-red")
    + kpi_card("Погоджені дані", f"{coverage}%", f"{submitted_count} із {total_active} заходів", "kpi-green" if coverage >= 70 else "kpi-yellow")
    + kpi_card("Критичний ризик", critical_count, f"середній + критичний: {critical_count + medium_count}", "kpi-red" if critical_count else "kpi-gray")
    + kpi_card("Без погоджених даних", without_data, pct_value(without_data, total_active), "kpi-red" if without_data else "kpi-green")
    + '</div>',
    unsafe_allow_html=True
)
...
if not goal_failure.empty:
    ...
    insights.append((
        "Найбільш проблемна стратегічна ціль",
        f"СЦ {row['goal_code']} має {int(row['Невиконаних'])} проблемних заходів із {int(row['Активних_заходів'])}. Вага у вибірці — {row['Вага_невиконання']}%."
    ))
if not dep_failure.empty:
    row = dep_failure.iloc[0]
    insights.append((
        "Підрозділ з найбільшою концентрацією ризику",
        f"{row['ssp_department']}: {int(row['Невиконаних'])} проблемних заходів із {int(row['Активних_заходів'])}. Вага у вибірці — {row['Вага_невиконання']}%."
    ))
insights.append((
    "Що робити першочергово",
    "Перевірити заходи без погоджених даних, потім — заходи з критичним ризиком і завершеним строком виконання."
))

st.markdown(
    '<div class="insight-grid">'
    + ''.join([f'<div class="insight"><strong>{title}</strong>{text}</div>' for title, text in insights[:3]])
    + '</div>',
    unsafe_allow_html=True
)
...
if show_overview or show_goals:
    st.markdown('<div class="panel"><div class="panel-title">Ключова картина виконання</div><div class="panel-subtitle">Менше графіків, але більше управлінського змісту: статуси, цілі та основні джерела ризику.</div>', unsafe_allow_html=True)
    c1, c2 = st.columns([0.9, 1.35])

    with c1:
        status_order = ["Виконано", "Частково виконано", "Виконується", "Не виконано", "Термін не настав", "Втратив актуальність"]
        status_counts["status_display"] = pd.Categorical(status_counts["status_display"], categories=status_order, ordered=True)
        status_counts = status_counts.sort_values("status_display")
        fig = px.bar(
            status_counts,
            x="Кількість",
            y="status_display",
            ...
            labels={"status_display": "Статус", "Кількість": "Кількість"}
        )
        fig.update_layout(showlegend=False)
        st.plotly_chart(apply_plot_theme(fig, height=360), use_container_width=True)

    with c2:
        goal_for_chart = goal_progress.sort_values(["Ризикових", "Активних_заходів"], ascending=[False, False]).copy()
        fig = px.bar(
            goal_for_chart,
            x="Виконання",
            y="Ціль",
            ...
            hover_data=["Активних_заходів", "Покриття_%", "Ризикових", "Критичних", "Середній_ризик"],
            title="Стратегічні цілі: виконання і ризик",
            labels={"Виконання": "Виконання, %", "Ціль": "Стратегічна ціль"}
        )
...
if show_risks:
    st.markdown('<div class="panel"><div class="panel-title">Ризики</div><div class="panel-subtitle">Цей блок показує, де саме накопичуються критичні та середні ризики.</div>', unsafe_allow_html=True)
    ...
    with c2:
        ...
        fig = px.scatter(
            ...
            color_continuous_scale="Reds",
            title="Підрозділи: виконання, ризик і масштаб портфеля",
            labels={"Виконання": "Виконання, %", "Середній_ризик": "Середній risk score"}
        )
...
if show_dynamics:
    ...
    for y in years_for_calc:
        for q in quarters_for_calc:
            ...
            trend_rows.append({
                ...
                "Ризикові заходи, %": calc_risk_share(temp)
            })
    ...
    if not trend_df.empty:
        fig = px.line(
            trend_df,
            x="Період",
            y=["Виконання", "Покриття погодженими даними", "Ризикові заходи, %"],
            markers=True,
            title="Тренд виконання, покриття та ризику",
...
if not presentation_mode and focus_mode in ["Огляд", "Ризики", "Таблиці"]:
    ...
    if risk_table.empty:
        ...
        st.dataframe(
            risk_table[["Період", "Код", "Захід", "Індикатор", "Головний ССП", "Статус", "План", "Факт", "Ризик", "Risk score", "Причина ризику", "Опис прогресу"]],
