...
# Крок 1: сам iframe компонента ховаємо, щоб він НЕ створював окремий блок унизу.
# Саму пташку і чат далі JS-ом переносимо в parent.document, тобто в основний DOM Streamlit.
st.markdown("""
<style>
/* Ховаємо технічний iframe компонента, але залишаємо виконання JavaScript */
...
# Крок 2: components.html використовується тільки як носій JavaScript.
# Пташка і вікно чату створюються в основному документі сторінки, тому:
# - вони поверх усієї сторінки;
# - клік по пташці працює;
# - прозорий шар не блокує роботу з іншими елементами сторінки.
...
<script>
(function(){
  var SU  = __SUPABASE_URL_JS__;
  var SK  = __SUPABASE_KEY_JS__;
  var ME  = "Керівник ССП";
  var COL = {"Керівник ССП":"#3b82f6","Координатор":"#16a34a"};
  var LET = {"Керівник ССП":"К","Координатор":"⚙"};
  var cache = {
    coordinator: __MSGS_COORD_JS__,
    responsible: __MSGS_RESP_JS__,
    group: __MSGS_GROUP_JS__
  };
  var cur = "coordinator";
  var poll = null;
  ...
  /* При кожному rerun прибираємо попередню версію, щоб не дублювалось */
  var oldRoot = doc.getElementById("bird-chat-root");
  if (oldRoot) oldRoot.remove();
  var oldStyle = doc.getElementById("bird-chat-style");
...
#bird-fab:hover {
  ...
  box-shadow: 0 12px 36px rgba(37,99,235,.6) !important;
}
#bird-fab svg {
...
.bmw { display: flex !important; flex-direction: column !important; }
.bmw.me { align-items: flex-end !important; }
.bmw.other { align-items: flex-start !important; }
.bmr { display: flex !important; align-items: flex-end !important; gap: 7px !important; }
.bmw.me .bmr { flex-direction: row-reverse !important; }
.bav {
...
.bmt { font-size: 10px !important; color: #94a3b8 !important; margin-top: 3px !important; }
.bmw.me .bmt { text-align: right !important; }
...
#bcinp button:hover { filter: brightness(1.1) !important; }
@media (max-width: 700px) {
  #bird-fab { right: 20px !important; bottom: 82px !important; }
  #bird-chat { right: 14px !important; bottom: 154px !important; width: calc(100vw - 28px) !important; }
}
`;
...
`;
  ...
  function toggle(){
    ...
    if (open) {
      ...
      sb();
      startPoll();
      setTimeout(function(){ $("#bci").focus(); }, 100);
    } else {
      stopPoll();
    }
  }
...
_chat_html = (
    _chat_html
    .replace("__SUPABASE_URL_JS__", _json.dumps(_SUPABASE_URL))
    .replace("__SUPABASE_KEY_JS__", _json.dumps(_SUPABASE_KEY))
    .replace("__MSGS_COORD_JS__", _msgs_coord)
    .replace("__MSGS_RESP_JS__", _msgs_resp)
    .replace("__MSGS_GROUP_JS__", _msgs_group)
)
