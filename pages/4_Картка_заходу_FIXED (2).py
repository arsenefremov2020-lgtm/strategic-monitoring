...
def financing_html(row):
    ...
    if has_state_budget:
        ...
        <div class="finance-block">
            ...
            <div class="finance-kpkvk">КПКВК&nbsp;{escape(kpkvk) if kpkvk else "—"}</div>
...
if view_mode in ["Огляд", "Індикатор прогресу"]:
    ...
    with c1:
        if exec_status == "Втратило актуальність":
            st.info("Захід втратив актуальність — прогрес не відображається.")
        elif exec_status == "Не настав час":
            st.info("Термін виконання ще не настав або дані не подано.")
        else:
            st.plotly_chart(
    ...
    with c2:
        ...
        st.write(
            ...
            "Якщо план = 'х' або факт не подано — відображається статус 'Не настав час'. "
            "Якщо одиниця виміру — 'так/ні': факт = 'так' → 100%, 'ні' → 0%. "
            "Числово: (факт ÷ план) × 100%, де >100% = Виконано, 75–100% = Частково, <75% = Не виконано."
        )

        if exec_status not in ["Втратило актуальність", "Не настав час"]:
...
if view_mode in ["Огляд", "Квартальна динаміка"]:
    ...
    # Build bar chart data for plotly
    bar_labels = []
    bar_facts = []
    bar_plans = []
    bar_colors = []
    ...
    for q in quarters:
        ...
        bar_labels.append(f"{q} кв.")
        bar_plans.append(plan_num_val)

        # Quarter card HTML
        # Compute quarter-level progress bar width
        q_pct = 0
        if bar_facts and bar_facts[-1] is not None and plan_num_val and plan_num_val > 0:
            q_pct = min(round((bar_facts[-1] / plan_num_val) * 100), 100)
    ...
    # Plotly bar chart
    if any(v is not None for v in bar_facts):
        fig = go.Figure()
        ...
        # Fact bars
        fact_vals = [v if v is not None else 0 for v in bar_facts]
        fig.add_trace(go.Bar(
            x=bar_labels,
            y=fact_vals,
            name="Фактичне значення",
            marker_color=bar_colors,
            text=[str(v) if v else "" for v in bar_facts],
